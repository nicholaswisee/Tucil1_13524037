...
\subsubsection{Algoritma \textit{Heuristic Search}}

Algoritma \textit{Heuristic Search} termasuk dalam kategori algoritma
\textit{brute-force}. Cara kerja \textit{heuristic search} sangat lempang
...
Inti dari \textit{exhaustic search} dapat dibagi menjadi tiga langkah, yaitu:
...
Repository diorganisir mengikuti konvensi standar Maven/Gradle untuk proyek
...
Class \texttt{Position} adalah value object immutable yang merepresentasikan
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item Menerima dan memproses command-line arguments jika ada
  \item Memisahkan definisi UI (FXML) dari inisialisasi runtime
\end{itemize}
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item Rendering simbol queen pada posisi solusi
  \item Adaptasi ukuran sel berdasarkan dimensi canvas
  \item Custom drawing dengan kontrol penuh atas rendering
\end{itemize}
...
\begin{itemize}[leftmargin=1.5em]
  \item Parsing argumen command-line atau prompt input dari pengguna
  ...
  \item Menampilkan hasil ke console dengan format user-friendly
...
\begin{enumerate}[label=\arabic*., leftmargin=2em]
  \item \textbf{Temuan 1} --- deskripsi temuan pertama dari eksperimen.
  \item \textbf{Temuan 2} --- deskripsi temuan kedua dari eksperimen.
  \item \textbf{Temuan 3} --- deskripsi temuan ketiga dari eksperimen.
\end{enumerate}

\subsection{Saran Pengembangan}
\begin{enumerate}[label=\arabic*., leftmargin=2em]
  \item Implementasi algoritma \textit{backtracking} untuk meningkatkan efisiensi pencarian.
  \item Penambahan heuristik seperti \textit{constraint propagation} untuk mengurangi ruang pencarian.
  \item Optimisasi multithreading untuk memanfaatkan multiple CPU cores.
  \item Implementasi mode batch processing untuk menyelesaikan multiple puzzle secara bersamaan.
\end{enumerate}

\subsection{Refleksi dan Saran}

Tugas ini keren! Jujur, ga \textit{expect} kalo Tucil 1 ini bakal menggunakan
permainan yang masih cukup relevan di zaman sekarang, dan lewat Tucil ini saya
belajar banyak. Terima kasih aku sampaikan kepada asisten yang telah memberikan
...
\item LinkedIn, ``Queens Puzzle,'' 2024. [Online]. Available: \url{https://www.linkedin.com/games/queens/}.

\item R. Munir, \textit{Diktat Kuliah IF2211 Strategi Algoritma}. Bandung: Program Studi Teknik Informatika ITB, 2024.

\item T. H. Cormen, C. E. Leiserson, R. L. Rivest, and C. Stein, \textit{Introduction to Algorithms}, 3rd ed. Cambridge, MA: MIT Press, 2009.

\item Oracle, ``JavaFX Documentation,'' 2024. [Online]. Available: \url{https://openjfx.io/}.
...
\item Gradle Inc., ``Gradle User Manual,'' 2024. [Online]. Available: \url{https://docs.gradle.org/}.
